...
def _scientific_proposal(prepared: Any) -> SemanticProposal:
    ...
    for index, series in enumerate(scientific_series):
        ...
        # originpro-cli-plot integration, 2026-09-02: bin counts remain support data.
        # Declare the existing renderer's scaled distribution bar separately from the
        # observed calibration line, with source lineage and explicit helper approval.
        calibration_count = (
            getattr(series, "size_column", None) if plot_kind == "calibration_curve" else None
        )
        if calibration_count in item_ids:
            count_helper = f"derived_calibration_count_{index:03d}"
            derived_items.append(
                DerivedDataItem(
                    item_id=count_helper,
                    semantic_role="bin_count_scaled_to_bottom_12_percent",
                    disposition=DataDisposition.RENDER_SECONDARY,
                    operation_id="scale_by_constant",
                    input_item_ids=(item_ids[calibration_count],),
                    confidence=1.0,
                    parameters=(("factor", spec.calibration_count_scale),),
                    evidence_codes=("calibration_distribution_display_contract",),
                )
            )
            elements.append(
                FigureElement(
                    element_id=f"calibration_distribution_{index:03d}",
                    element_kind="bar",
                    data_item_ids=(*anchor_ids, count_helper),
                    required=True,
                    axis="left",
                    legend_label="Prediction distribution (display scaled)",
                )
            )
        auxiliary = _unique_existing(
            (
                getattr(series, "error_column", None),
                None if calibration_count is not None else getattr(series, "size_column", None),
